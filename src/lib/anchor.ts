...
// Create provider
export const createProvider = (wallet: anchor.Wallet) => {
...
export const createProgram = (provider: anchor.AnchorProvider) => {
...
export const getSlotPDA = (slotNumber: number) => {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("slot"), new anchor.BN(slotNumber).toArrayLike(Buffer, "le", 2)],
    PROGRAM_ID
  );
};

// IDL (Interface Definition Language) - this should match your compiled program
export const IDL = {
  "version": "0.1.0",
  "name": "meme_wall",
  "instructions": [
